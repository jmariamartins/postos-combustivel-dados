...
OUTPUT_PATH = Path("public/data/Postos.csv")
HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; PostosCSV-updater/1.0)"}


def get_fuel_ids() -> list[tuple[int, str]]:
    """Fetch all available fuel types from the API and return list of (id, name)."""
    url = f"{BASE_URL}/GetCombustiveis?idioma=0"
    resp = requests.get(url, headers=HEADERS, timeout=30)
    resp.raise_for_status()
    data = resp.json()
    # Response is typically a list of {"Id": 3, "Nome": "Gasolina simples 95", ...}
    ...
    print(f"Combustíveis disponíveis: {len(result)}")
    for fid, fname in result:
        print(f"  [{fid}] {fname}")
    return result
...
def main():
    ...
    if not fuels:
        print("ERRO: Não foi possível obter lista de combustíveis.", file=sys.stderr)
        sys.exit(1)

    print("\nA carregar postos por combustível...")
    all_rows = []
    for fuel_id, fuel_name in fuels:
        print(f"  {fuel_name} (id={fuel_id})", end=" ")
